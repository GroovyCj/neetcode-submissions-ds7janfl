class Solution:
    ...
    def dfs(self, grid: List[List[int]], r: int, c: int, color: int,original_color: int) -> None:
            ROWS, COLS = len(grid), len(grid[0])

            if not (0 <= r <  ROWS and 0 <= c < COLS) or grid[r][c] != original_color:
                return
    ...
    def floodFill(self, image: List[List[int]], sr: int, sc: int, color: int) -> List[List[int]]:
        ...
        if original_color != color:
            self.dfs(grid=image, r=sr, c=sc, color=color, original_color=original_color)        
        return image
